...
describe('loadCSV', () => {
	it('should load and parse CSV file correctly', async () => {
		await fs.promises.writeFile(TEST_FILEPATH, 'Resource,Source\nResource1,Source1\nResource2,Source2\nResource3,Source3');
	...
	it('should throw an error if CSV file contains commas in values', async () => {
		...
		await expect(loadCSV(TEST_FILEPATH)).rejects.toThrow(
			'Commas are not allowed in the values of the CSV file'
		);
	});
});
