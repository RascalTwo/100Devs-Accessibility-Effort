...
describe('generateChangelogContentLIs', () => {
	it('should generate changelog content LIs', () => {
		const generateHTMLForDateAndCommitsMock = jest
			.mocked(generateHTMLForDateAndCommits)
			.mockImplementation((day) => `<li>${day}</li>`);
		...
		expect(changelogContentLIs).toEqual(['<li>2024-03-17</li>', '<li>2024-03-15</li>']);
	});
});
